...
The test file is written by hand with random values. It covers all specified instructions above. Some possible mistakes in value behavior and component wiring are commented in the test program. For each instruction, an accompanying comment shows the register/memory value after executing.